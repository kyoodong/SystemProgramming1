...
static int assem_pass2(void)
{
	...
	for (int tokenIndex = 0; tokenIndex < token_line; tokenIndex++) {
		...
		if (!strcmp(t->operator, "START")) {
			...
			while (literalCount) {
				literalCount--;
				if (literals[literalCount][1] == 'C') {
					for (int c = 3; literals[literalCount][c] != '\''; c++) {
						printf("%X", literals[literalCount][c]);
					}
				}
				printf("\n");
			}
			...
			if (t->operand[0] == 'X') {
				for (int k = 2; t->operand[0][k] != '\''; k++) {
					printf("%c", t->operand[0][k]);
				}
			}
		}
		else if (!strcmp(t->operator, "WORD")) {
			printf("%06X", atoi(t->operand[0]));
		}
		else if (!strcmp(t->operator, "EQU")) {
		}
		else {
			...
			if (instruction.format == 3) {
				...
				if (t->operand[0][0] == '#') {
					...
					t->nixbpe |= N;
					strcpy(t->operand[0], t->operand[0] + 1);
				}
